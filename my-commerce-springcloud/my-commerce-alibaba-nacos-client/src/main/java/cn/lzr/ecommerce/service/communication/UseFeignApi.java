...
import org.springframework.cloud.openfeign.support.SpringMvcContract; // 用于支持和MVC一样的注解
...
public class UseFeignApi {
    ...
    public JwtToken thinkingInFeign(UsernameAndPassword usernameAndPassword){
        ...
        // 核心就是进行 Feign 客户端初始化, 必须要配置 encoder、decoder、contract
        AuthorityFeignClient feignClient = Feign.builder()  // 1. Feign 默认配置初始化
                ...
                .contract(new SpringMvcContract())          // 用于支持和MVC一样的web注解
                .target(                                    // 3 生成代理对象
